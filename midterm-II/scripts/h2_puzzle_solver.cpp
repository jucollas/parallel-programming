...
bool is_valid(const pair<int, int>& cord) {
    return 0 <= cord.first && cord.first < N && 0 <= cord.second && cord.second < M;
}
  ...
  for(int i = 0; i < N; ++i){
    ...
    for(int j = 0; j < M; ++j){
      if(board[k] != EMPTY_VALUE)
        ans += manhattan(u, pos_goal[char_to_index(board[k])]);
      u.second += 1;
      k++;
    }
    u.first += 1;
...
/*int calculeHeuristics(const string& board, const pair<int, int>& cord, const pair<int, int>& new_cord, int new_index, int h_paret){
    int token_index = char_to_index(board[new_index]);
    int ans = h_paret - manhattan(cord, pos_goal[token_index]) + manhattan(new_cord, pos_goal[token_index]);;
    return ans;
}*/
...
void precompu(){
    for(int i = 0; i < N; ++i){
        for(int j = 0; j < M; ++j){
            pos_goal.push_back(std::make_pair(i, j));
        }
    }
}
