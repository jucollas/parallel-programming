...
void max_single_vector(int id, vector<vector<int>>* arrs, vector<int>* maximed){
  for(int i = 0; i < (int)(*arrs)[id].size(); ++i){
    (*maximed)[id] = max((*maximed)[id], (*arrs)[id][i]);
  }
}

void* max_mult_vector(void * args){
  data_t* data = (data_t*) args;
  for(int i = data->start; i < data->end; ++i){
    max_single_vector(i, data->arrs, data->maximed);
  }
  return NULL;
}

vector<int> max_arrs(vector<vector<int>>& arrs){
  int n = arrs.size();
  ...
  for(int i = 0; i < num_threads; ++i){
    ...
    pthread_create(&threads[i], NULL, max_mult_vector, &data[i]);
    curr = to;
  } 
  for(int i = 0; i < num_threads; ++i){
...
int main(){
  ...
  vector<int> maximed = max_arrs(arrs);
  for(int i = 0; i < n_arrs; ++i){
    cout << maximed[i] << ' '; 
  }
  cout << endl;
