...
// ------------------------------------------------------------
// UTILIDADES DEL VIEWER PARA LA VERSIÓN PARALELA
// ------------------------------------------------------------

void compute_stats_parallel(Generation& g, int& rabbits, int& foxes, int& rocks){
  rabbits = foxes = rocks = 0;
  Mat& eco = g.get_ecosystem();
  for(int i = 0; i < eco.size(); ++i){
    for(int j = 0; j < eco[i].size(); ++j){
...
void render_step_parallel(Generation& g, int step,
...
// ------------------------------------------------------------
// VIEWER PARALELO – INTERACTIVO
// ------------------------------------------------------------

void run_viewer_parallel(Generation& start, int N_GEN_TOTAL){
  ...
  // --- Nueva funcionalidad ---
  int want_view;
  cout << "¿Cuántas generaciones desea visualizar? ";
  cin >> want_view;

  int speed_level;
  cout << "Indique velocidad (1 = lento, 10 = rápido): ";
  cin >> speed_level;

  // Normalización de velocidad
  speed_level = max(1, min(10, speed_level));
  int delay_ms = 1100 - speed_level * 100;

  // Limitar viewer al rango permitido
  int max_view = min(want_view, N_GEN_TOTAL);

  vector<int> hist_r, hist_f, hist_k;

  for(int step = 0; step < max_view; ++step){

    // --- Estadísticas ---
    int rabbits, foxes, rocks;
    compute_stats_parallel(*current, rabbits, foxes, rocks);
    hist_r.push_back(rabbits);
    hist_f.push_back(foxes);
    hist_k.push_back(rocks);

    // --- Render ---
    render_step_parallel(*current, step, rabbits, foxes, rocks, delay_ms);

    // Espera según velocidad
    this_thread::sleep_for(chrono::milliseconds(delay_ms));

    // --- Avance de 1 generación usando tu motor paralelo ---
  ...
  dump_stats_csv_parallel(hist_r, hist_f, hist_k, "stats_viewer_parallel.csv");

  cout << "\nVisualización terminada. Estadísticas guardadas en stats_viewer_parallel.csv\n";
}
...
int main(){
  ...
  run_viewer_parallel(start, N_GEN);
