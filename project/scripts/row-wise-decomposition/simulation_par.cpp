...
class data_t{
  ...
  data_t(int _start, 
    ...
    data_t(int _start, int _end, Mat* _ecosystem):
    start(_start),
    end(_end),
...
void delete_entities(Mat& ecosystem){
  ...
  for(int i = 0; i < num_threads; ++i){
    ...
    thdata[i] = data_t(act, to, &ecosystem);
 ...
 */
void modify_ecosystem(const int lower_limit, const int upper_limit, const int id_thread, const EntityType type, Entity* entity, Mat& ecosystem){
 ...
 */
void modify_ecosystem_copy(const int lower_limit, const int upper_limit, const int id_thread, const EntityType type, Entity* entity, Mat& ecosystem){
  const int id_row = entity->get_x();
  const int ind_mutex = must_blocked(id_row, id_thread, lower_limit, upper_limit);
  const bool block = ind_mutex != -1;
 ...
 */
void move_single_fox(const int x, const int y, const int G, const int start, const int end, const int id_thread, const Mat& current, Mat& next, const Param& param ){    
  ...
  if(can_procreate){
    ...
    modify_ecosystem(start, end, id_thread, EntityType::FOX, baby, next); 
  }
  ...
  modify_ecosystem(start, end, id_thread, EntityType::FOX, act_mov, next); 
}
 ...
 */
void move_single_rabbit(int x, int y, int G, const int start, const int end, const int id_thread, const Mat& current, Mat& next, const Param& param){
  const Entity& act = *current[x][y].get_entity();
  ...
  if (can_procreate) {
    ...
    Entity* baby = new Entity(act.get_x(), act.get_y());
    modify_ecosystem(start, end, id_thread, EntityType::RABBIT, baby, next); 
  }

  Entity* act_mov = new Entity(step.first, step.second, update_proc);
  modify_ecosystem(start, end, id_thread, EntityType::RABBIT, act_mov, next); 
}
 ...
 */
void copy_entity(const int x, const int y, const int lower, const int upper, const int id_thr, const Mat& current, Mat& next){
  Entity* entity = new Entity(*current[x][y].get_entity());
  EntityType type = current[x][y].get_type();
  modify_ecosystem_copy(lower, upper, id_thr, type, entity, next);
}
 ...
 */
void move_sigle_entity(const int x, const int y, const int G, const int start, const int end, const int id_thread, const EntityType turn, const Mat& current, Mat& next, const Param& param){
  EntityType type = current[x][y].get_type();
  ...
  if(turn == EntityType::RABBIT){
    if(type == EntityType::RABBIT){
      move_single_rabbit(x, y, G, start, end, id_thread, current, next, param);
    }else if(type == EntityType::FOX){
      copy_entity(x, y, start, end, id_thread, current, next);
    }
  }else if(turn == EntityType::FOX){
    if(type == EntityType::FOX){
      move_single_fox(x, y, G, start, end, id_thread, current, next, param);
 ...
 * @param param Parámetros globales.
 */
void move_single_row(const int id_row, const int G, const int start, const int end, const int id_thread, const EntityType turn, const Mat& current, Mat& next, const Param& param){
  for(int i = 0; i < sz(current[id_row]); ++i){
    move_sigle_entity(id_row, i, G, start, end, id_thread, turn, current, next, param);
  }
}
...
void* move_mult_row(void * args){
  ...
  for(int row = start; row < end; ++row){
    move_single_row(row, G, start, end, id_thread, turn, current, next, param);
  }
  return NULL;
}
...
void move_entities(Generation* current, Generation* next, const EntityType turn){
  ...
  for(int i = 0; i < num_threads; ++i){
    int mv = (i < rem) ? 1 : 0;
    int to = act + len_chunk + mv;
    thdata[i] = data_t(act, to, current_gen, i, turn, &current_ecosystem, &next_ecosystem, &param);
